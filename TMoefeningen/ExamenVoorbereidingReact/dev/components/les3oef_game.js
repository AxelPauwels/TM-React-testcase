...
import InputForm from './les3oef_input_form';

const Vraag = (props) => {
    return (
        <p>{(props.huidigeVraagIndex + 1)}. Which is the capital of {props.huidigeVraag.country}?</p>
    );
};
...
class Game extends Component {
    ...
    controleerAntwoord(event, antwoord) {
        // event.target.value === antwoord (beide zijn hetzelfde, bv "tokyo") => gewoon een test op props door te geven
    ...
    addHighscore(naam) {
        ...
        this.props.history.push('/les3b/highscores');
    }
        ...
        if (this.state.eindeSpel) {
            ...
            return (
                <div>
                    ...
                    <Link to="/les3b">Stop playing</Link>
                </div>
            );
        }
    }
