...
const Stars = (props) => {
    ...
    let baseUrl = "http://movieserver.hopto.org/react";
    ...
    for (let i = 0; i < props.rating; i++) {
        stars.push(
            <img id={"StarOn" + (i+1)} key={"StarOn" + (i+1)} src={`${baseUrl}/images/StarOn.png`} style={imgStyle}/>
        );
    }

    for (let i = 0; i < aantalLegeSterren; i++) {
        stars.push(
            <img id={"StarOff" + (i+1)} key={"StarOff" + (i+1)} src={`${baseUrl}/images/StarOff.png`} style={imgStyle}/>
        );
    }
